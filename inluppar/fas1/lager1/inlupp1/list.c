...
//typedef struct tree tree_t; in header file
//typedef struct list_list_t; in header file
...
struct list{              //själva listan

  node_t *first;
  node_t *last;
};

struct node{                   //länkarna
  void *elem;        // elem stored in the node. void * we can store whatever we like
  node_t *next;     // pointer pointing to the next node in the list(for linked list), null for last node
};
...
list_t *list_new()
{
  list_t *plist = malloc(sizeof(struct list)); //plist is pointer
  if (plist == NULL)
    return NULL;
  plist->first = NULL;
  plist->last =NULL;
...
node_t *node_new(void *element, node_t *next)
{
  node_t *pnode = malloc(sizeof(struct node));
  if (pnode == NULL)
    return NULL;
  pnode->elem = element;
  pnode->next = next;
...
{
  node_t *newElem = node_new(elem, list);
